...
/**
 * Represente un duel entre deux personnages dont une liche.
 *
 */
public class DuelLifeSteal extends Interaction<VuePersonnage> {
	 ...
	 */
	public DuelLifeSteal(Arene arene, VuePersonnage attaquant, VuePersonnage defenseur) {
		super(arene, attaquant, defenseur);
	}
	
	public DuelLifeSteal(){
	...
	public void interagit() {
		try {
			...
			if (perteVie > 0) {
				...
				arene.incrementeCaractElement(attaquant, Caracteristique.VIE, perteVie);
				
				logs(Level.INFO, Constantes.nomRaccourciClient(attaquant) + " fait un suçon ("
						+ perteVie + " points de vie absorbés) a " + Constantes.nomRaccourciClient(defenseur));
			}
